...
filedir = os.path.join(cwd,"Repli_all")

# read names of files with specific file extension (vhdr) and of a specific scan condition (EC,EO)
filesEC = [filedir +'\\'+ f for f in os.listdir(filedir) 
         if all(x in f for x in ("vhdr", "rest", "EC"))]


# initializing lists for storing IDs, values and variable names of subjects
IDs_subjects = []
all_ica=[]
eeg_all=[]  

# loop over all subjects (filenames saved in EC)
for f in filesEC: 
    ...
    # rename channels to fit standard
    raw.rename_channels({'FP1' : 'Fp1', 'FP2' : 'Fp2', 'SO1' : 'S01', 'SO2' : 'S02'})
    
    # remap non-eeg channels 
    raw.set_channel_types(mapping={'S01': 'eog', 'S02': 'eog','M1':'misc', 'M2':'misc'})

    # drop non-EEG channels
    raw.drop_channels(['S01','S02','M1','M2']) 
    ...
    # ICA
    ica = mne.preprocessing.ICA(n_components=30).fit(raw_temp)
...
for axes_row in axes:
    ...
    for ax in axes_row:
        if cnt < bads_all.shape[1]:
            ...
            cnt +=1
fig.tight_layout()
plt.savefig('bad_ica_components_repli.jpg', format='jpg', dpi = 1200)

# find componets similar to determined bad compnetns (corrmap) in all subjects' ICAs 
...
# get indexes channels that correspond to channels of the main sample
with open("channel_names", "rb") as fp:
    channels_main = pickle.load(fp)

channels_repli = data_all[0].info.ch_names

idx_channels_main = []
for ch in channels_main:
    idx_channels_main.append(np.where(ch == np.array(channels_repli))[0])
idx_channels_main = np.array(idx_channels_main).flatten()
...
for s in range(len(data_all)):
    ...
    #define range from 10 to 300 seconds
    data = data[:,10*freq_resample:300*freq_resample]
    
    # window size
    step = 2*freq_resample;

    # looking for bad epochs that hav at least one value higher than max value
    ...
    # take only channels used in the main sample
    data_concat = data_concat[idx_channels_main,:]
    
    no_bad_epochs = data.shape[1]/step - data_epoched.shape[0]
    
    # subjects that survived the exclusion criteria (<50 of epochs bad)
...
for d in data_epoched_all:
    ...
    no_bad_epochs.append(data.shape[1]/step - d.shape[0])
    
no_bad_epochs = np.array(no_bad_epochs)        

idx_bad_epochs = np.where(np.array(no_bad_epochs) >= 50)[0]
...
maps_group = np.load('microstates_group.npy') # load group microstates computed on main sample
...
for st, e in zip(n_single_occurences_states_all, epochs_rejected_all):
    freq = st / (145-e) / 2 # occurences divided by 2*epochs (1 epoch = 2 s)
...
for s in range(len(MSE_all)):
    # averaging MSE of each spatial and temporal cluster
    MSE_s = MSE_all[s]
    MSE_cluster_s = []
...
df_complexity.to_pickle("./df_complexity_repli.pkl")
